...
JELLYFIN_TRIGGER_PERCENTAGE = float(
    os.getenv('JELLYFIN_TRIGGER_PERCENTAGE', '50.0'))
JELLYFIN_POLL_INTERVAL = int(
    os.getenv(
        'JELLYFIN_POLL_INTERVAL',
        '900'))  # Default 15 minutes (900 seconds)
...
missing_handler.setFormatter(logging.Formatter(
    '%(asctime)s - %(levelname)s - %(message)s'))
...
def setup_cleanup_logging():
    ...
    cleanup_formatter = logging.Formatter(
        '%(asctime)s - %(levelname)s - %(message)s')
    ...
    try:
        ...
        for rule_name, rule_details in config['rules'].items():
            ...
            if str(series_id) in series_dict:
                ...
                updated = True
                logger.info(
                    f"📺 Updated CONFIG activity for series {series_id}: S{season_number}E{episode_number} at {
        ...
        if updated:
            save_config(config)
            logger.info(
                f"✅ Config saved - series {series_id} now has complete activity data")
        else:
            logger.warning(
                f"Series {series_id} not found in any rule for activity update")

    except Exception as e:
        logger.error(
            f"Error updating activity date for series {series_id}: {
                str(e)}")


def get_activity_date_with_hierarchy(
        series_id,
        series_title=None,
    ...
    """
    logger.info(
        f"🔍 Getting activity date for series {series_id} ({series_title})")
            ...
            if activity_date:
                if return_complete:
                    ...
                    if last_season and last_episode:
                        logger.info(
                            f"✅ Using complete config data for series {series_id}: S{last_season}E{last_episode} at {
                                datetime.fromtimestamp(activity_date)}")
                        return activity_date, last_season, last_episode
                    else:
                        logger.info(
                            "Config has activity_date but missing season/episode data")
                        # Continue to external sources for complete data
                else:
                    logger.info(
                        f"✅ Using config activity date for series {series_id}: {
                            datetime.fromtimestamp(activity_date)}")
    ...
    if not series_title:
        try:
            ...
            logger.warning(
                f"Failed to get Sonarr title for series {series_id}: {
                    str(e)}")

    # Step 2: Check external services (only if title available)
    if series_title:
        # Check which external service is configured (user typically has one,
        # not both)
        ...
        # Prefer Tautulli if both are configured (since it's more accurate for
        # watch tracking)
        if tautulli_url and tautulli_api_key:
            logger.info(f"🔍 Checking Tautulli for '{series_title}'")

            # Use enhanced Tautulli function
            tautulli_result = get_tautulli_last_watched(
                series_title, return_complete=return_complete)
            if tautulli_result:
                if return_complete and isinstance(tautulli_result, tuple):
                    timestamp, season, episode = tautulli_result
                    logger.info(
                        f"✅ Using complete Tautulli data for series {series_id}: S{season}E{episode} at {
                            datetime.fromtimestamp(timestamp)}")
                    return timestamp, season, episode
                elif not return_complete:
                    logger.info(
                        f"✅ Using Tautulli date for series {series_id}: {
                            datetime.fromtimestamp(tautulli_result)}")
            ...
            jellyfin_result = get_jellyfin_last_watched(
                series_title, return_complete=return_complete)
            if jellyfin_result:
                if return_complete and isinstance(jellyfin_result, tuple):
                    timestamp, season, episode = jellyfin_result
                    logger.info(
                        f"✅ Using complete Jellyfin data for series {series_id}: S{season}E{episode} at {
                            datetime.fromtimestamp(timestamp)}")
                    return timestamp, season, episode
                elif not return_complete:
                    logger.info(
                        f"✅ Using Jellyfin date for series {series_id}: {
                            datetime.fromtimestamp(jellyfin_result)}")
                    return jellyfin_result
            logger.info(f"⚠️  No Jellyfin date found for series {series_id}")

        else:
            logger.info(
                "No external watch tracking configured (Tautulli/Jellyfin)")
    ...
    if sonarr_date:
        if return_complete:
            logger.info(
                f"✅ Using Sonarr file date with S1E1 fallback for series {series_id}: {
                    datetime.fromtimestamp(sonarr_date)}")
            return sonarr_date, 1, 1  # Default fallback
        else:
            logger.info(
                f"✅ Using Sonarr file date for series {series_id}: {
                    datetime.fromtimestamp(sonarr_date)}")
            return sonarr_date
...
def get_server_activity():
    ...
    try:
        ...
        if all([series_title, season_number, episode_number]):
            return series_title, int(season_number), int(
                episode_number), thetvdb_id, themoviedb_id
        ...
        logger.error(
            f"Failed to read or parse data from server webhook: {
                str(e)}")
...
def get_series_id(series_name, thetvdb_id=None, themoviedb_id=None):
    ...
    try:
            ...
            try:
                ...
                for series in series_list:
                    if series.get('tvdbId') == tvdb_id_int:
                        logger.info(
                            f"Found TVDB ID match: {
                                series['title']} (TVDB: {thetvdb_id})")
                        return series['id']
            ...
            try:
                ...
                for series in series_list:
                    if series.get('tmdbId') == tmdb_id_int:
                        logger.info(
                            f"Found TMDB ID match: {
                                series['title']} (TMDB: {themoviedb_id})")
                        return series['id']
            except (ValueError, TypeError):
                pass
        ...
        for series in series_list:
            sonarr_title_clean = re.sub(
                r'\s*\(\d{4}\)$', '', series['title']).strip()
            if sonarr_title_clean.lower() == webhook_title_clean.lower():
                logger.info(
                    f"Found match ignoring year: '{
                        series['title']}' matches '{series_name}'")
                return series['id']
        ...
        if close_matches:
            missing_logger.info(
                f"Series not found in Sonarr: '{series_name}'. Possible matches: {close_matches}")
        else:
            missing_logger.info(
                f"Series not found in Sonarr: '{series_name}'. No close matches.")
        return None
...
def monitor_episodes(episode_ids, monitor=True):
    ...
    if response.ok:
        ...
        logger.error(
            f"Failed to set episodes {action}. Response: {
                response.text}")
...
def trigger_episode_search_in_sonarr(episode_ids):
    ...
    if response.ok:
        ...
        logger.error(
            f"Failed to send episode search command. Response: {
                response.text}")
...
def fetch_next_episodes_dropdown(
        series_id,
        season_number,
        episode_number,
        get_type,
    ...
    try:
        if get_type == "all":
            ...
            current_season_episodes = get_episode_details(
                series_id, season_number)
            remaining_current = [
                ep['id'] for ep in current_season_episodes if ep['episodeNumber'] > episode_number]
            ...
            logger.info(
                f"Dropdown seasons mode: Found {
                    len(next_episode_ids)} episodes across {seasons_to_get} seasons")
            return next_episode_ids
            ...
            while len(next_episode_ids) < num_episodes:
                next_season_episodes = get_episode_details(
                    series_id, current_season_num)
                if not next_season_episodes:
                    logger.info(
                        f"No more episodes available after season {
                            current_season_num - 1}")
                    break

                remaining_needed = num_episodes - len(next_episode_ids)
                next_episode_ids.extend(
                    [ep['id'] for ep in next_season_episodes[:remaining_needed]])
                current_season_num += 1

                # Prevent infinite loops
                if current_season_num > season_number + 10:
                    logger.warning("Stopping after checking 10 seasons ahead")
                    break

            logger.info(
                f"Dropdown episodes mode: Found {
                    len(next_episode_ids)} out of {num_episodes} requested")
...
def get_tautulli_last_watched(series_title, return_complete=False):
    ...
    try:
        ...
        if not tautulli_url or not tautulli_api_key:
            logger.warning("Tautulli not configured")
            return None
        ...
        title_variations = [
            ...
            series_title.split(" (")[0],
            # Before parentheses
        ]

        # Try each variation (but limit API calls)
        for search_title in set(
                title_variations[:3]):  # Limit to top 3 variations
            ...
            if (normalized_entry == normalized_search
                or normalized_entry in normalized_series_title
                    or normalized_series_title in normalized_entry):
                ...
                if last_watched:
                    try:
                        ...
                        if return_complete:
                            ...
                            season_num = most_recent.get(
                                'parent_media_index')  # Season number
                            episode_num = most_recent.get(
                            ...
                            if season_num and episode_num:
                                ...
                                logger.info(
                                    f"Found complete Tautulli data for '{entry_title}': S{season}E{episode} at {
                                        datetime.fromtimestamp(timestamp)}")
                                return timestamp, season, episode
                            else:
                                # Fallback to timestamp with default
                                # season/episode
                                logger.info(
                                    f"Found Tautulli timestamp for '{entry_title}' with S1E1 fallback: {
                                        datetime.fromtimestamp(timestamp)}")
                                return timestamp, 1, 1
                        else:
                            # Existing behavior - just timestamp
                            logger.info(
                                f"Found Tautulli watch for '{entry_title}': {
                                    datetime.fromtimestamp(timestamp)}")
                            return timestamp
        ...
        if response.ok:
            ...
            for user in users:
                if user.get('Name', '').lower() == username.lower():
                    user_id = user.get('Id')
                    logger.info(
                        f"Found Jellyfin User ID for '{username}': {user_id}")
                    return user_id

            logger.warning(
                f"Username '{username}' not found in Jellyfin users")
            ...
            logger.warning(
                f"Failed to get Jellyfin users: {
                    response.status_code}")
...
def get_jellyfin_last_watched(series_title, return_complete=False):
    ...
    try:
        jellyfin_url = os.getenv('JELLYFIN_URL')
        jellyfin_api_key = os.getenv('JELLYFIN_API_KEY')
        jellyfin_user_input = os.getenv(
            'JELLYFIN_USER_ID')  # Could be username or GUID

        if not all([jellyfin_url, jellyfin_api_key, jellyfin_user_input]):
            logger.warning("Jellyfin not configured")
            return None

        # Check if the user input is already a GUID (contains hyphens) or a
        # username
        if '-' in jellyfin_user_input and len(jellyfin_user_input) > 30:
            ...
            logger.debug(
                f"Converting username '{jellyfin_user_input}' to User ID")
            jellyfin_user_id = get_jellyfin_user_id(
                jellyfin_url, jellyfin_api_key, jellyfin_user_input)

            if not jellyfin_user_id:
                logger.warning(
                    f"Could not find User ID for username '{jellyfin_user_input}'")
                return None
        ...
        if return_complete:
            ...
            logger.debug(
                f"Getting complete Jellyfin data for '{series_title}'")
            ...
            if response.ok:
                ...
                for item in items:
                    ...
                    if (normalized_series_title == normalized_item
                        or normalized_series_title in normalized_item
                            or normalized_item in normalized_series_title):

                        user_data = item.get('UserData', {})
                        last_played = user_data.get('LastPlayedDate')
                        season_number = item.get('ParentIndexNumber')  # Season
                        episode_number = item.get(
                            'IndexNumber')       # Episode
                        ...
                        if last_played and season_number and episode_number:
                            try:
                                ...
                                logger.info(
                                    f"Found complete Jellyfin data for '{series_name}': S{season}E{episode} at {dt}")
                                return timestamp, season, episode

                            except (ValueError, TypeError) as e:
                                logger.warning(
                                    f"Invalid Jellyfin date format: {last_played} - {e}")
                                continue
        ...
        if not response.ok:
            logger.warning(
                f"Jellyfin API error: {response.status_code} - {response.text[:200]}")
            return None
        ...
        logger.debug(
            f"Jellyfin found {
                len(items)} series for user {jellyfin_user_input}")
            ...
            if (normalized_series_title == normalized_item
                ...
                if last_played:
                    try:
                        ...
                        if return_complete:
                            logger.info(
                                f"Found Jellyfin series LastPlayedDate for '{item_name}' with S1E1 fallback: {dt}")
                            return timestamp, 1, 1  # Default fallback for series-level date
                        else:
                            logger.info(
                                f"Found Jellyfin LastPlayedDate for '{item_name}': {dt}")
                            return timestamp

                    except ValueError as e:
                        logger.warning(
                            f"Invalid Jellyfin LastPlayedDate format: {last_played} - {e}")
                        continue
...
def get_sonarr_latest_file_date(series_id):
    ...
    try:
        ...
        if not response.ok:
            logger.error(
                f"Failed to get episode files for series {series_id}: {
                    response.status_code}")
            return None
        ...
        for file_data in episode_files:
            ...
            if episodes:
                ...
                ep_display = f"E{
                    min(episode_numbers)}" if episode_numbers else "E?"
            else:
                ep_display = "E?"

            date_added_str = file_data.get('dateAdded')
            logger.debug(
                f"S{season}{ep_display}: dateAdded = '{date_added_str}'")

            if not date_added_str:
                logger.warning(f"Missing dateAdded for S{season}{ep_display}")
                continue

            timestamp = parse_date_fixed(
                date_added_str, f"S{season}{ep_display}")
            ...
            if timestamp:
                ...
                logger.error(
                    f"Failed to parse dateAdded for S{season}{ep_display}: '{date_added_str}'")

        if latest_file_date:
            logger.info(
                f"Latest file: {latest_episode_info} at {
            ...
            logger.warning(
                f"No valid episode file dates found for series {series_id}")
            return None
...
def find_episodes_leaving_keep_block(
        all_episodes,
        keep_type,
        keep_count,
        last_watched_season,
    ...
    try:
        ...
        if keep_season_premieres and episodes_leaving:
            ...
            episodes_leaving = [
                ep for ep in episodes_leaving if ep['episodeNumber'] != 1]
            if original_count != len(episodes_leaving):
                logger.info(
                    f"Protected {original_count - len(episodes_leaving)} "
                    "season premiere(s) from deletion")
...
def process_episodes_for_webhook(
        series_id,
        season_number,
        episode_number,
        rule):
    ...
    try:
        logger.info(
            f"Processing webhook for series {series_id}: S{season_number}E{episode_number}")
        ...
        if not current_episode:
            logger.error(
                f"Could not find current episode S{season_number}E{episode_number}")
            return
        ...
        episodes_leaving_keep_block = find_episodes_leaving_keep_block(
            ...
            season_number,
            episode_number,
            keep_season_premieres)

        if episodes_leaving_keep_block:
            episode_file_ids = [ep['episodeFileId']
                                for ep in episodes_leaving_keep_block if 'episodeFileId' in ep]
            if episode_file_ids:
                ...
                logger.info(
                    f"Immediately deleted {
                        len(episode_file_ids)} episodes leaving keep block")
...
def check_time_based_cleanup(series_id, rule):
    ...
    try:
        ...
        if dormant_days and days_since_activity > dormant_days:
            return True, f"DORMANT: {
                days_since_activity:.1f}d > {dormant_days}d"
        elif grace_watched and days_since_activity > grace_watched:
            return True, f"GRACE WATCHED: {
                days_since_activity:.1f}d > {grace_watched}d"
        else:
            return False, f"PROTECTED: {
                days_since_activity:.1f}d since activity"
...
def delete_episodes_in_sonarr_with_logging(
    ...
    if dry_run:
        print(
            f"🔍 DRY RUN: Would delete {
                len(episode_file_ids)} episode files from {series_title}")
        print(
            f"🔍 DRY RUN: Episode file IDs: {episode_file_ids[:5]}{'...' if len(episode_file_ids) > 5 else ''}")
        return
    ...
    print(
        f"📊 Deletion summary: {successful_deletes} successful, {
...
def run_grace_watched_cleanup():
    ...
    try:
        cleanup_logger.info(
            "🟡 GRACE WATCHED CLEANUP: Checking inactive series")
        ...
        for rule_name, rule in config['rules'].items():
            ...
            cleanup_logger.info(
                f"📋 Rule '{rule_name}': Checking grace_watched ({grace_watched_days} days, min retention: {
                    min_retention_days or 'none'})")
            ...
            for series_id_str, series_data in series_dict.items():
                try:
                    ...
                    result = get_activity_date_with_hierarchy(
                        series_id, series_title, return_complete=True)
                    ...
                    if not activity_date:
                        cleanup_logger.debug(
                            f"⏭️ {series_title}: No activity date from any source, skipping")
                        continue

                    # Check if grace period has expired
                    days_since_activity = (
                        current_time - activity_date) / (24 * 60 * 60)

                    if days_since_activity > grace_watched_days:
                        cleanup_logger.info(
                            f"🟡 {series_title}: Inactive for {
                                days_since_activity:.1f} days > {grace_watched_days} days")
                        cleanup_logger.info(
                            f"   📺 Last watched: S{last_season}E{last_episode}")

                        # Get all episodes for this series
                        all_episodes = fetch_all_episodes(series_id)

                        # FIXED: Find watched episodes (UP TO AND INCLUDING
                        ...
                        for episode in all_episodes:
                            ...
                            # This INCLUDES the last watched episode (delete
                            # what you've already seen)
                            if (season_num < last_season
                                    or (season_num == last_season
                                        and episode_num <= last_episode)):
                                watched_episodes.append(episode)

                        # SEEDING PROTECTION: Filter by minimum retention if
                        # configured
                        ...
                        if min_retention_days:
                            ...
                            for ep in watched_episodes:
                                days_since_added = get_days_since_file_added(
                                    ep)
                            ...
                            if protected_count > 0:
                                cleanup_logger.info(
                                    f"   🛡️ Protected {protected_count} files (seeding < {min_retention_days} days)")

                        # Get episode file IDs for deletion
                        episode_file_ids = [
                            ep['episodeFileId'] for ep in eligible_episodes if 'episodeFileId' in ep]
                        ...
                        if episode_file_ids:
                            ...
                            cleanup_logger.info(
                                f"   ⏭️ No watched episodes to delete (last watched: S{last_season}E{last_episode})")
                    else:
                        cleanup_logger.debug(
                            f"🛡️ {series_title}: Protected - only {days_since_activity:.1f} days since activity")

                except (ValueError, TypeError) as e:
                    cleanup_logger.error(
                        f"Error processing series {series_id_str}: {
                            str(e)}")
                    continue

        cleanup_logger.info(
            f"🟡 Grace watched cleanup: Deleted {total_deleted} episodes")
...
def run_grace_unwatched_cleanup():
    ...
    try:
        cleanup_logger.info(
            "⏰ GRACE UNWATCHED CLEANUP: Checking inactive series")
        ...
        for rule_name, rule in config['rules'].items():
            ...
            min_retention_days = rule.get('min_retention_days')
            cleanup_logger.info(
                f"📋 Rule '{rule_name}': Checking grace_unwatched ({grace_unwatched_days} days, min retention: {
                    min_retention_days or 'none'})")
            rule_dry_run = rule.get('dry_run', False)
            is_dry_run = global_dry_run or rule_dry_run
            ...
            for series_id_str, series_data in series_dict.items():
                try:
                    ...
                    if days_since_activity > grace_unwatched_days:
                        cleanup_logger.info(
                            f"⏰ {series_title}: Inactive for {
                                days_since_activity:.1f} days > {grace_unwatched_days} days")
                        cleanup_logger.info(
                            f"   📺 Last watched: S{last_season}E{last_episode}")
                        ...
                        # FIXED: Find unwatched episodes (STRICTLY AFTER last
                        # watched position)
                        unwatched_episodes = []
                        for episode in all_episodes:
                            if not episode.get('hasFile'):
                            ...
                            # Episode is "unwatched" if it's STRICTLY AFTER the
                            # last watched position
                            if (season_num > last_season
                                    or (season_num == last_season
                                        and episode_num > last_episode)):
                        ...
                        if episode_file_ids:
                            ...
                            cleanup_logger.info(
                                f"   ⏭️ No unwatched episodes to delete after S{last_season}E{last_episode}")
                    else:
                        cleanup_logger.debug(
                            f"🛡️ {series_title}: Protected - only {days_since_activity:.1f} days since activity")
        ...
        cleanup_logger.info(
            f"⏰ Grace unwatched cleanup: Deleted {total_deleted} episodes")
        return total_deleted
...
def run_dormant_cleanup():
    ...
    try:
        ...
        if storage_min_gb:
            ...
            cleanup_logger.info(
                "⏰ No storage gate - running scheduled dormant cleanup")
        ...
        for rule_name, rule in config['rules'].items():
            ...
            for series_id_str in rule.get('series', {}):
                try:
                    ...
                    activity_date = get_activity_date_with_hierarchy(
                        series_id, series_info['title'])
                    if not activity_date:
                        continue
                    ...
                    if days_since_activity > dormant_days:
                        ...
                        # configured
                        eligible_episodes = [
                            ep for ep in all_episodes if ep.get('hasFile') and 'episodeFileId' in ep]
                        if min_retention_days:
                            filtered_episodes = []
                            protected_count = 0
                            ...
                            if protected_count > 0:
                                cleanup_logger.info(
                                    f"   {series_info['title']}: Protected "
                                    f"{protected_count} files "
                                    f"(seeding < {min_retention_days} days)")
        ...
        for candidate in candidates:
            ...
            if storage_min_gb and not candidate['is_dry_run']:
                ...
                if current_disk and current_disk['free_space_gb'] >= storage_min_gb:
                    cleanup_logger.info("Storage target reached")
                    break

            cleanup_logger.info(
                f"🔴 {
                    candidate['title']}: Dormant for {
                    candidate['days_since_activity']:.1f} days")
            delete_episodes_in_sonarr_with_logging(
        ...
        cleanup_logger.info(
            f"🔴 Dormant cleanup: Processed {processed_count} series")
...
def check_global_storage_gate():
    ...
    try:
        ...
        if current_free_gb < storage_min_gb:
            return True, storage_min_gb, f"Storage gate OPEN: {
                current_free_gb:.1f}GB < {storage_min_gb}GB threshold"
        else:
            return False, storage_min_gb, f"Storage gate CLOSED: {
                current_free_gb:.1f}GB >= {storage_min_gb}GB threshold"
...
def get_sonarr_disk_space():
    ...
    try:
        ...
        if response.ok:
            ...
            if diskspace_data:
                main_disk = max(
                    diskspace_data, key=lambda x: x.get(
                        'totalSpace', 0))
...
def run_unified_cleanup():
    ...
    try:
        ...
        if storage_min_gb:
            ...
            if not current_disk:
                cleanup_logger.error(
                    "❌ Cannot get disk space - aborting cleanup")
                return 0

            if current_disk['free_space_gb'] >= storage_min_gb:
                cleanup_logger.info(
                    f"🔒 Storage gate CLOSED: {
                        current_disk['free_space_gb']:.1f}GB >= {storage_min_gb}GB threshold")
            ...
            cleanup_logger.info(
                f"🔓 Storage gate OPEN: {
                    current_disk['free_space_gb']:.1f}GB < {storage_min_gb}GB threshold")
            cleanup_logger.info(
                f"🎯 Target: Clean until back above {storage_min_gb}GB")
            storage_gated = True
        else:
            # No storage gate - always run
            cleanup_logger.info(
                "⏰ No storage gate - running all cleanup functions")
            storage_gated = False
        ...
        cleanup_logger.info(
            "🔴 Phase 1: Dormant cleanup (delete ALL episodes from abandoned series)")
        ...
        if storage_gated and dormant_count > 0:
            ...
            if current_disk and current_disk['free_space_gb'] >= storage_min_gb:
                cleanup_logger.info(
                    f"🎯 TARGET REACHED after dormant: {
                        current_disk['free_space_gb']:.1f}GB >= {storage_min_gb}GB")
                cleanup_logger.info("✅ Stopping cleanup - goal achieved")
                return total_processed

        # PRIORITY 2: GRACE WATCHED (delete watched episodes from inactive
        # series)
        cleanup_logger.info(
            "🟡 Phase 2: Grace watched cleanup (delete watched episodes from inactive series)")
        watched_count = run_grace_watched_cleanup()
        total_processed += watched_count
        cleanup_logger.info(
            f"🟡 Grace watched result: {watched_count} operations")
        ...
        if storage_gated and watched_count > 0:
            ...
            if current_disk and current_disk['free_space_gb'] >= storage_min_gb:
                cleanup_logger.info(
                    f"🎯 TARGET REACHED after grace watched: {
                        current_disk['free_space_gb']:.1f}GB >= {storage_min_gb}GB")
                cleanup_logger.info("✅ Stopping cleanup - goal achieved")
                return total_processed
        ...
        cleanup_logger.info(
            "⏰ Phase 3: Grace unwatched cleanup (delete unwatched episodes past deadline)")
        unwatched_count = run_grace_unwatched_cleanup()
        total_processed += unwatched_count
        cleanup_logger.info(
            f"⏰ Grace unwatched result: {unwatched_count} operations")
        ...
        if final_disk:
            cleanup_logger.info(
                f"💾 Final free space: {
                    final_disk['free_space_gb']:.1f}GB")
            if storage_gated:
...
def process_jellyfin_episode(episode_info):
    ...
    try:
        ...
        with LAST_PROCESSED_LOCK:
            ...
            last_processed_time = LAST_PROCESSED_JELLYFIN_EPISODES.get(
                episode_key)

            if last_processed_time and last_processed_time > five_minutes_ago:
                logger.info(
                    f"⏭️ Skipping duplicate processing for {episode_key} within 5 minutes")
                return False
            else:
                LAST_PROCESSED_JELLYFIN_EPISODES[episode_key] = current_time

        logger.info(
            f"🎯 Processing Jellyfin episode at {
                episode_info['progress_percent']:.1f}%: {series_name} S{season_number}E{episode_number}")
        ...
        logger.info(
            f"✅ Jellyfin polling processing complete for {series_name} S{season_number}E{episode_number}")
        return True
...
def poll_jellyfin_session(session_id, initial_episode_info):
    ...
    logger.info(
        f"   📺 {
            initial_episode_info['series_name']} S{
            initial_episode_info['season_number']}E{
                initial_episode_info['episode_number']}")
    logger.info(
        f"   🎯 Will trigger at {JELLYFIN_TRIGGER_PERCENTAGE}% progress")
    ...
    try:
        ...
        while session_id in active_polling_sessions and not processed:
            ...
            if not current_session:
                logger.info(
                    f"📺 Session {session_id} ended - stopping polling (poll #{poll_count})")
                break

            # Extract current episode info
            current_episode_info = extract_episode_info_from_session(
                current_session)

            if not current_episode_info:
                logger.info(
                    f"⏭️ Session {session_id} no longer playing episode - stopping polling")
                break

            # Check if we're still on the same episode
            if (current_episode_info['series_name'] != initial_episode_info['series_name']
                or current_episode_info['season_number'] != initial_episode_info['season_number']
                    or current_episode_info['episode_number'] != initial_episode_info['episode_number']):
                logger.info(
                    f"📺 Episode changed in session {session_id} - stopping polling for original episode")
                break
            ...
            if should_trigger_processing(
                ...
                logger.info(
                    f"🎯 Trigger threshold reached! Processing at {
                        current_progress:.1f}%")
                ...
                if success:
                    processed = True
                    logger.info(
                        f"✅ Successfully processed - stopping polling for session {session_id}")
                else:
                    logger.warning(
                        "Processing failed - continuing polling")
        ...
        if not processed and session_id not in active_polling_sessions:
            logger.info(
                f"🔄 Polling stopped for session {session_id} - session ended before trigger")

    except Exception as e:
        logger.error(
            f"❌ Error in Jellyfin polling thread for session {session_id}: {
                str(e)}")
...
def start_jellyfin_polling(session_id, episode_info):
    ...
    with polling_lock:
        ...
        logger.info(
            f"🎬 Starting Jellyfin polling for: {
                episode_info['series_name']} S{
                episode_info['season_number']}E{
...
def stop_jellyfin_polling(session_id):
    ...
    with polling_lock:
        if session_id in active_polling_sessions:
            logger.info(
                f"🛑 Stopping Jellyfin polling for session {session_id}")
            del active_polling_sessions[session_id]
            return True
        return False
...
def handle_jellyfin_playback_start(webhook_data):
    ...
    try:
        ...
        # Check your webhook payload for correct field
        ...
        if not all([session_id,
                    series_name,
                    season_number is not None,
                    episode_number is not None]):
            logger.warning(
                f"Missing required fields in Jellyfin start webhook: {webhook_data}")
            return False
...
def get_jellyfin_active_episodes():
    ...
    try:
        ...
        if not response.ok:
            logger.warning(
                f"Failed to get Jellyfin sessions: {
                    response.status_code}")
            return []
        ...
        for session in sessions:
            ...
            # Skip if paused (optional - you might want to process paused
            # episodes too)
            if play_state.get('IsPaused', False):
                logger.debug(
                    f"Skipping paused episode: {
                        now_playing.get('SeriesName')} S{
                        now_playing.get('ParentIndexNumber')}E{
...
def should_process_episode(episode_info):
    ...
    if episode_key in processed_episodes:
        ...
        if last_processed_time > four_hours_ago:
            logger.debug(
                f"Already processed {episode_key} recently at {
                    last_processed_progress:.1f}%")
            return False

    # Process if progress is above threshold
    if progress >= JELLYFIN_TRIGGER_PERCENTAGE:
        logger.info(
            f"🎯 Episode ready for processing: {
                episode_info['series_name']} S{
                episode_info['season_number']}E{
                episode_info['episode_number']}")
        logger.info(
            f"   📊 Progress: {
                progress:.1f}% >= {JELLYFIN_TRIGGER_PERCENTAGE}%")
        logger.info(
            f"   👤 User: {
                episode_info['user_name']} ({
                episode_info['device_name']})")
        return True
...
def process_jellyfin_episode_active_polling(episode_info):
    ...
    try:
        ...
        with LAST_PROCESSED_LOCK:
            five_minutes_ago = current_time - (5 * 60)
            last_processed_time = LAST_PROCESSED_JELLYFIN_EPISODES.get(
                jellyfin_episode_key)

            if last_processed_time and last_processed_time > five_minutes_ago:
                logger.info(
                    f"⏭️ Skipping duplicate processing for {jellyfin_episode_key} within 5 minutes")
                return False
            else:
                LAST_PROCESSED_JELLYFIN_EPISODES[jellyfin_episode_key] = current_time

        logger.info(
            f"🎯 Processing Jellyfin episode: {series_name} S{season_number}E{episode_number}")
        ...
        logger.info(
            f"✅ Jellyfin processing complete for {series_name} S{season_number}E{episode_number}")
        return True
...
def cleanup_old_processed_episodes():
    ...
    try:
        ...
        if old_episodes:
            logger.info(
                f"🧹 Cleaned up {
                    len(old_episodes)} old processed episode records")
...
def jellyfin_polling_loop():
    ...
    logger.info(
        f"Jellyfin active polling started (every "
        f"{JELLYFIN_POLL_INTERVAL // 60} minutes)")
    logger.info(
        f"🎯 Will process episodes at {JELLYFIN_TRIGGER_PERCENTAGE}% progress")
    ...
    while jellyfin_polling_running:
        try:
            ...
            if not active_episodes:
                ...
                for episode_info in active_episodes:
                    if should_process_episode(episode_info):
                        success = process_jellyfin_episode_active_polling(
                            episode_info)
                        if success:
                            processed_count += 1
                ...
                if processed_count > 0:
                    logger.info(
                        f"✅ Processed {processed_count} episodes this cycle")
                else:
                    logger.info(
                        "⏭️ No episodes ready for processing this cycle")
            ...
            # Wait for next cycle
            logger.info(
                f"Next check in {JELLYFIN_POLL_INTERVAL // 60} minutes")
            time.sleep(JELLYFIN_POLL_INTERVAL)
        ...
        if series_id:
            ...
            if rule:
                process_episodes_for_webhook(
                    series_id, season_number, episode_number, rule)
            else:
                update_activity_date(series_id, season_number, episode_number)
    else:
