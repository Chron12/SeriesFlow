...
SEERR_ENABLED = bool(
    (JELLYSEERR_URL and JELLYSEERR_API_KEY) or (
...
if app.config['TMDB_API_KEY']:
    app.logger.info(
        "TMDB_API_KEY is set - request system will function normally")
else:
    app.logger.warning(
        "TMDB_API_KEY is missing - you may encounter issues fetching series details and seasons")
...
stream_handler.setLevel(
    logging.DEBUG if os.getenv(
        'FLASK_DEBUG',
        'false').lower() == 'true' else logging.INFO)
    ...
    try:
        ...
        try:
            ...
            if response.ok:
                ...
                downloading = len(
                    [r for r in records if r.get('status') == 'downloading'])
                queued = len([r for r in records if r.get(
                    'status') in ['queued', 'delay']])
...
class OCDarrScheduler:
    ...
    def update_interval_from_settings(self):
        ...
        try:
            ...
            self.cleanup_interval_hours = global_settings.get(
    ...
    def start_scheduler(self):
        ...
        print(
            f"✓ Global storage gate scheduler started - cleanup every {
    ...
    def _run_cleanup(self):
        try:
            ...
            result = subprocess.run(["python3", os.path.join(
                os.getcwd(), "media_processor.py")], capture_output=True, text=True)
    ...
    def get_status(self):
        ...
        if self.last_cleanup == 0:
            ...
            next_time = self.last_cleanup + \
                (self.cleanup_interval_hours * 3600)
            next_cleanup = datetime.fromtimestamp(
        ...
        return {
            ...
            "last_cleanup": datetime.fromtimestamp(
                self.last_cleanup).strftime("%Y-%m-%d %H:%M:%S") if self.last_cleanup else "Never",
...
def setup_cleanup_logging():
    ...
    cleanup_file_formatter = logging.Formatter(
...
def get_sonarr_series():
    ...
    try:
        ...
        if not response.ok:
            app.logger.error(
                f"Failed to fetch series from Sonarr: {
                    response.status_code}")
            return []
        ...
        tags_response = requests.get(
            f"{sonarr_url}/api/v3/tag", headers=headers)
        if tags_response.ok:
            ...
            if watched_tag_id:
                ...
                app.logger.debug(
                    f"Filtered out {len(all_series) - len(filtered_series)}"
                    " series with 'watched' tag")
...
def index():
    ...
    default_rule = (list(config['rules'].keys())[0]
                    if config['rules'] else 'default')
    return render_template('rules_index.html',
                           config=config,
                           all_series=all_series,
                           sonarr_stats=sonarr_stats,
                           SONARR_URL=sonarr_url,
                           current_rule=request.args.get('rule', default_rule))
...
def create_rule():
    ...
    if request.method == 'POST':
        ...
        get_count = None if get_type == 'all' else int(
            get_count) if get_count else 1
        keep_count = None if keep_type == 'all' else int(
            keep_count) if keep_count else 1
        ...
        min_retention_days = None if not min_retention_days else int(
            min_retention_days)
...
def edit_rule(rule_name):
    ...
    if request.method == 'POST':
        ...
        if 'set_as_default' in request.form:
            ...
            # Set the first available rule as default, or remove default if
        ...
        if 'set_as_default' in request.form and config.get(
...
def assign_rules():
    ...
    for series_id in series_ids:
        for rule, details in config['rules'].items():
            series_dict = details.get('series', {})
            if series_id in series_dict and isinstance(
                    series_dict[series_id], dict):
    ...
    for series_id in series_ids:
        if series_id in existing_activity:
            ...
            target_series_dict[series_id] = {
                'activity_date': None}  # New series
...
def inject_service_urls():
    ...
    default_icons = {
        # TV icon for Sonarr (TV show management)
        'sonarr': 'fas fa-tv',
        ...
        # Chart icon for Tautulli (analytics)
        'tautulli': 'fas fa-chart-bar',
        # Download icon for SABnzbd (downloader)
        'sabnzbd': 'fas fa-download',
        # Advanced search icon for Prowlarr (indexer)
        'prowlarr': 'fas fa-search-plus',
        # Video icon for Radarr (movie management)
        'radarr': 'fas fa-video'
    }
    ...
    for key, value in os.environ.items():
        if key.endswith('_URL') and value:
            ...
            # Get name: Use corresponding _NAME variable, default_names, or
            # capitalize service_id
            name_key = f'{key[:-len("_URL")]}_NAME'
            service_name = os.getenv(
                name_key, default_names.get(
                    service_id, service_id.capitalize()))

            # Get icon: Use corresponding _ICON variable, default_icons, or
            # generic icon
            icon_key = f'{key[:-len("_URL")]}_ICON'
            service_icon = os.getenv(
                icon_key, default_icons.get(
                    service_id, 'fas fa-link'))
...
def set_default_rule():
    ...
    return redirect(
        url_for(
            'index',
            message=f"Set '{rule_name}' as default rule"))
...
def unassign_series():
    ...
    return redirect(
        url_for(
            'index',
            message=f"Unassigned {
                len(series_ids)} series from all rules"))
    ...
    try:
        ...
        for series_id in series_ids:
            ...
            if unmonitor_episodes:
                success = episeerr_utils.unmonitor_series(
                    int(series_id), headers)
                if success:
                    unmonitored_count += 1
...
def cleanup_config_rules():
    ...
    try:
        ...
        existing_series_ids = set(str(series['id'])
                                  for series in existing_series)
        changes_made = False
        for rule_name, rule_details in config['rules'].items():
            original_count = len(rule_details.get('series', {}))
            rule_details['series'] = {
                series_id: details for series_id, details in rule_details.get(
                    'series', {}).items() if series_id in existing_series_ids}
            if len(rule_details['series']) != original_count:
                removed_count = original_count - len(rule_details['series'])
                app.logger.info(
                    f"Cleaned up rule '{rule_name}': Removed {removed_count} non-existent series")
                changes_made = True
        config['rules'] = {rule: details for rule, details in config['rules'].items(
...
def cleanup():
    ...
    return redirect(
        url_for(
            'index',
            message="Configuration cleaned up successfully"))
...
def api_series_with_status():
    ...
    try:
        ...
        for series in all_series:
            ...
            unmanaged_info = unmanaged_series.get(
                series_id) if is_unmanaged else None
...
def scheduler_status():
    ...
    try:
        if 'cleanup_scheduler' not in globals():
            return jsonify(
                {"status": "error", "message": "Scheduler not initialized"}), 500
...
def update_global_settings():
    ...
    try:
        ...
        auto_assign_new_series = data.get(
...
def scheduler_status_global():
    ...
    try:
        ...
        for rule_name, rule in config['rules'].items():
            has_cleanup = rule.get('grace_watched') or rule.get(
                'grace_unwatched') or rule.get('dormant_days')
            if has_cleanup:
    ...
    if request.method == 'POST':
        try:
            ...
            for rule_name in config.get('rules', {}).keys():
                ...
                app.logger.info(
                    f"Setting {rule_name} dry_run to: {rule_dry_run}")
            ...
            return redirect(
                url_for(
                    'scheduler_admin',
                    message="Dry run settings saved successfully"))
        except Exception as e:
            current_app.logger.error(
                f"Error saving dry run settings: {
                    str(e)}")
            return redirect(
                url_for(
                    'scheduler_admin',
                    message=f"Error saving settings: {
                        str(e)}"))
    try:
        config = load_config()
        global_dry_run = os.getenv(
        ...
        return redirect(
            url_for(
                'scheduler_admin',
                message=f"Error loading settings: {
                    str(e)}"))
...
def render_simple_logs_page(logs_or_message):
    ...
    if isinstance(logs_or_message, str):
        ...
        content = ('<div style="font-family: monospace; font-size: 0.9em; '
                   'max-height: 600px; overflow-y: auto; border: 1px solid '
                   '#333; padding: 1rem; background: #1a1a1a;">')
...
def select_seasons(tmdb_id):
    ...
    try:
        ...
        if not show_data:
            return render_template(
                'error.html',
                message="Failed to get show details from TMDB")
        ...
        formatted_show = {
            ...
            'name': show_data.get(
                'name',
                'Unknown Series'),
            'overview': show_data.get(
                'overview',
                ''),
            'posterUrl': f"https://image.tmdb.org/t/p/w300{
                show_data['poster_path']}" if show_data.get('poster_path') else '/static/placeholder-banner.png',
            'seasons': []}
        ...
        return render_template(
            'error.html',
            message=f"Error loading season selection: {
                str(e)}")
...
def select_episodes(tmdb_id):
    ...
    try:
        ...
        try:
            selected_seasons = [
                int(s.strip()) for s in selected_seasons_param.split(',') if s.strip()]
        except ValueError:
            app.logger.error(
                f"Invalid seasons parameter: {selected_seasons_param}")
        ...
        app.logger.info(
            f"Episode selection for TMDB ID {tmdb_id}, seasons: {selected_seasons}")
                ...
                try:
                    with open(os.path.join(REQUESTS_DIR, filename), 'r') as f:
                        ...
                        if str(request_data.get('tmdb_id')) == str(tmdb_id):
                            # Try both 'series_id' and 'id' fields (different
                            # request types)
                            series_id = request_data.get(
                                'series_id') or request_data.get('id')
                            request_id = request_data.get(
                                'request_id') or request_data.get('id')
                            tvdb_id = request_data.get('tvdb_id')
                            app.logger.info(
                                f"Found matching request: series_id="
                                f"{series_id}, request_id={request_id}, "
                                f"tvdb_id={tvdb_id}")
                            break
                except Exception as e:
                    app.logger.error(
                        f"Error reading request file {filename}: {
                            str(e)}")

        if not request_id:
            return render_template(
                'error.html',
                message="No pending request found for this series")

        # If series_id is not in the request file, look it up from Sonarr using
        # TVDB ID
        if not series_id and tvdb_id:
            app.logger.info(
                f"Looking up series_id in Sonarr for TVDB ID: {tvdb_id}")
                ...
                if response.ok:
                    ...
                    for series in all_series:
                        if str(series.get('tvdbId')) == str(tvdb_id):
                            ...
                            app.logger.info(
                                f"Found series_id {series_id} for TVDB ID {tvdb_id}")
                            break
            except Exception as e:
                app.logger.error(
                    f"Error looking up series in Sonarr: {
                        str(e)}")

        if not series_id:
            return render_template(
                'error.html',
                message="Series not found in Sonarr. Please make sure the series is added.")
        ...
        app.logger.info(
            f"Rendering episode selection with selected_seasons: {selected_seasons}")
        ...
        return render_template(
            'error.html',
            message=f"Error loading episode selection: {
                str(e)}")
...
def process_episode_selection():
    ...
    try:
        ...
        app.logger.info(
            f"Form lists: episodes={
                request.form.getlist('episodes')}")

        # Get form data
        request_id = request.form.get('request_id')
        # Gets ALL values with name 'episodes'
        episodes = request.form.getlist('episodes')
        action = request.form.get('action')

        app.logger.info(
            f"Processing: request_id={request_id}, action={action}, episodes={episodes}")
        ...
        if action == 'cancel':
            ...
            if request_id:
                ...
                if os.path.exists(request_file):
                    ...
                    app.logger.info(
                        f"Cancelled and removed request {request_id}")
            ...
            for episode_str in episodes:
                try:
                    if ':' not in episode_str:
                        app.logger.error(
                            f"Invalid episode format (no colon): {episode_str}")
                        continue
                    ...
                    app.logger.warning(
                        f"Invalid episode format: {episode_str} - {str(e)}")
                    continue
            ...
            if not episodes_by_season:
                return redirect(
                    url_for(
                        'episeerr_index',
                        message="Error: No valid episodes found"))

            app.logger.info(
                f"Processing multi-season selection for series {series_id}: {episodes_by_season}")
            ...
            for season_number, episode_numbers in episodes_by_season.items():
                app.logger.info(
                    f"Processing Season {season_number}: {episode_numbers}")
                ...
                if success:
                    ...
                    app.logger.info(
                        f"✓ Successfully processed Season {season_number}")
                else:
                    failed_seasons.append(season_number)
                    app.logger.error(
                        f"✗ Failed to process Season {season_number}")
            ...
            if failed_seasons:
                ...
                message = f"Successfully processed {total_processed} episodes across {
                    len(seasons_list)} seasons"
                return redirect(url_for('episeerr_index', message=message))
        ...
        app.logger.error(
            f"Error processing episode selection: {
                str(e)}", exc_info=True)
        return redirect(
            url_for(
                'episeerr_index',
...
def get_tmdb_season(tmdb_id, season_number):
    ...
    try:
        ...
        if not season_data:
            return jsonify(
                {"error": "Failed to get season data from TMDB"}), 500
        ...
        for filename in os.listdir(REQUESTS_DIR):
            if filename.endswith('.json'):
                try:
                    with open(os.path.join(REQUESTS_DIR, filename), 'r') as f:
                        request_data = json.load(f)
                        # Add needs_season_selection flag if not present (for
                        # Jellyseerr requests)
...
def delete_request(tmdb_id):
    ...
    try:
            ...
            if filename.endswith('.json'):
                ...
                try:
                    ...
                    if str(request_data.get('tmdb_id')) == str(tmdb_id):
                        os.remove(filepath)
                        app.logger.info(
                            f"Deleted pending request file {filename} for TMDB ID {tmdb_id}")
                        deleted = True
                        break
                except Exception as e:
                    app.logger.error(
        ...
        if deleted:
            return jsonify(
                {"status": "success", "message": "Request deleted successfully"}), 200
        else:
            app.logger.warning(
                f"Pending request for TMDB ID {tmdb_id} not found")
            return jsonify(
                {"status": "error", "message": "Request not found"}), 404

    except Exception as e:
        app.logger.error(
            f"Error deleting request for TMDB ID {tmdb_id}: {
                str(e)}")
        return jsonify(
            {"status": "error", "message": "Failed to delete request"}), 500
...
def process_sonarr_webhook():
    ...
    try:
        ...
        app.logger.info(
            f"Processing series addition: {series_title} (ID: {series_id}, TVDB: {tvdb_id})")
        ...
        # This is more reliable than webhook data, especially if user sets
        # monitoring after adding
        ...
        try:
            ...
            if series_response.ok:
                ...
                app.logger.info(
                    f"Sonarr monitored seasons for {series_title}: {sonarr_monitored_seasons}")
            else:
                app.logger.warning(
                    f"Could not fetch series details from Sonarr: {
                        series_response.status_code}")
        except Exception as e:
            app.logger.error(
                f"Error fetching season monitoring from Sonarr: {
                    str(e)}")
        ...
        if not tags_response.ok:
            app.logger.error(
                f"Failed to get tags from Sonarr: {
                    tags_response.status_code}")
            return jsonify(
                {"status": "error", "message": "Failed to get tags"}), 500
        ...
        if not has_episeerr_default and not has_episeerr_select:
            ...
            import media_processor
            global_settings = media_processor.load_global_settings()
            auto_assign_enabled = global_settings.get(
                'auto_assign_new_series', False)

            if auto_assign_enabled:
                app.logger.info(
                    f"Auto-assign enabled: Adding {series_title} to default rule and monitoring first episode")

                # Set has_episeerr_default to True to trigger the full
                # processing flow
                has_episeerr_default = True
                app.logger.info(
                    f"Auto-assign treating {series_title} as if it has episeerr_default tag")
            else:
                app.logger.info(
                    f"Series {series_title} has no episeerr tags, doing nothing")
                return jsonify(
                    {"status": "success", "message": "Series has no episeerr tags, no processing needed"}), 200

        # Check for pending Jellyseerr request and get requested seasons
        jellyseerr_request_id = None
        # Will contain list of requested season numbers
        jellyseerr_requested_seasons = None
        tvdb_id_str = str(tvdb_id) if tvdb_id else None

        app.logger.info(
            f"Looking for Jellyseerr request with TVDB ID: {tvdb_id_str}")

        if tvdb_id_str:
            request_file = os.path.join(
                REQUESTS_DIR, f"jellyseerr-{tvdb_id_str}.json")
            if os.path.exists(request_file):
                try:
                    with open(request_file, 'r') as f:
                        request_data = json.load(f)
                    jellyseerr_request_id = request_data.get('request_id')
                    jellyseerr_requested_seasons = request_data.get(
                        'requested_seasons', [])
                    app.logger.info(
                        f"Found Jellyseerr request file: "
                        f"{jellyseerr_request_id}, Requested seasons: "
                        f"{jellyseerr_requested_seasons}")

                    # Cancel the Jellyseerr request
                    app.logger.info(
                        f"Cancelling Jellyseerr request {jellyseerr_request_id}")
                    cancel_result = episeerr_utils.delete_overseerr_request(
                        jellyseerr_request_id)
                    app.logger.info(
                        f"Jellyseerr cancellation result: {cancel_result}")

                    # Delete the file after processing
                    os.remove(request_file)
                    app.logger.info(
                        f"✓ Removed Jellyseerr request file for TVDB ID {tvdb_id_str}")

                except Exception as e:
                    app.logger.error(
                        f"Error processing Jellyseerr request file: {
                            str(e)}")
            else:
                app.logger.info(
                    f"No Jellyseerr request file found for TVDB ID: {tvdb_id_str}")
        ...
        if not unmonitor_success:
            app.logger.warning(
                f"Failed to unmonitor episodes for {series_title}")
        ...
        if removed_tags:
            ...
            if update_response.ok:
                app.logger.info(
                    f"✓ Removed tags {removed_tags} from {series_title}")
            else:
                app.logger.error(
                    f"Failed to remove tags from {series_title}: {
        ...
        if has_episeerr_default:
            app.logger.info(
                f"Processing {series_title} with episeerr_default tag")
            ...
            try:
                ...
                app.logger.info(
                    f"Adding {series_title} to default rule '{default_rule_name}'")

                if default_rule_name not in config['rules']:
                    app.logger.error(
                        f"Default rule '{default_rule_name}' not found in config!")
                    return jsonify(
                        {"status": "error", "message": f"Default rule '{default_rule_name}' not found"}), 500
                ...
                if not isinstance(series_dict, dict):
                    app.logger.warning(
                        f"Converting series from {
                            type(series_dict)} to dict for rule '{default_rule_name}'")
                    series_dict = {}
                ...
                if series_id_str not in series_dict:
                    ...
                    app.logger.info(
                        f"Added series {series_id_str} to rule '{default_rule_name}'")

                    # Validate config before saving
                    if len(config['rules']) < 1:
                        app.logger.error(
                            f"CONFIG CORRUPTION DETECTED! Only {len(config['rules'])} rules found")
                        return jsonify(
                            {"status": "error", "message": "Config corruption detected, save aborted"}), 500

                    save_config(config)
                    app.logger.info(
                        f"✓ Successfully added {series_title} to default rule '{default_rule_name}'")
                else:
                    app.logger.info(
                        f"Series {series_id_str} already in rule '{default_rule_name}'")

            except Exception as e:
                app.logger.error(
                    f"Error adding series to default rule: {
                        str(e)}", exc_info=True)
                return jsonify(
                    {"status": "error", "message": f"Failed to add series to rule: {str(e)}"}), 500
            ...
            try:
                ...
                app.logger.info(
                    f"Executing default rule with get_type '{get_type}', get_count '{get_count}' for {series_title}")
                ...
                if episodes_response.ok:
                    ...
                    if not season1_episodes:
                        app.logger.warning(
                            f"No Season 1 episodes found for {series_title}")
                    else:
                        # Determine which episodes to monitor based on get
                        # settings
                        episodes_to_monitor = []

                        if get_type == 'all':
                            # Get all episodes from all seasons
                            episodes_to_monitor = [ep['id']
                                                   for ep in all_episodes]
                            app.logger.info(
                                f"Monitoring all episodes for {series_title}")

                        elif get_type == 'seasons':
                            # Get all episodes from Season 1
                            episodes_to_monitor = [ep['id']
                                                   for ep in season1_episodes]
                            app.logger.info(
                                f"Monitoring Season 1 ({
                                    len(episodes_to_monitor)} episodes) for {series_title}")

                        else:  # episodes
                            try:
                                # INITIAL ADD: Always monitor first episode of EACH season
                                # This allows "sampling" each season instead of just Season 1
                                # The get_count setting will be used by
                                # Tautulli webhook for subsequent episodes
                                ...
                                # Priority: 1) Sonarr monitored seasons, 2)
                                # Jellyseerr requested seasons, 3) ALL seasons
                                seasons_to_monitor = sorted(
                                    seasons_dict.keys())
                                season_source = "all"

                                if sonarr_monitored_seasons:
                                    # Use Sonarr's season monitoring status
                                    # (from manual add)
                                    seasons_to_monitor = [
                                        s for s in seasons_to_monitor if s in sonarr_monitored_seasons]
                                    season_source = "sonarr"
                                    app.logger.info(
                                        f"Sonarr manual add: Using Sonarr monitored seasons {sonarr_monitored_seasons}")
                                elif jellyseerr_requested_seasons:
                                    # Use Jellyseerr requested seasons
                                    seasons_to_monitor = [
                                        s for s in seasons_to_monitor if s in jellyseerr_requested_seasons]
                                    season_source = "jellyseerr"
                                    app.logger.info(
                                        f"Jellyseerr request: Using requested seasons {jellyseerr_requested_seasons}")

                                for season_num in seasons_to_monitor:
                                    season_episodes = sorted(
                                        seasons_dict[season_num], key=lambda x: x.get(
                                            'episodeNumber', 0))
                                    if season_episodes:
                                        episodes_to_monitor.append(
                                            season_episodes[0]['id'])

                                if season_source == "sonarr":
                                    app.logger.info(
                                        f"Initial add (Sonarr): Monitoring "
                                        f"first episode of monitored seasons "
                                        f"{seasons_to_monitor} for "
                                        f"{series_title}")
                                elif season_source == "jellyseerr":
                                    app.logger.info(
                                        f"Initial add (Jellyseerr): "
                                        f"Monitoring first episode of "
                                        f"requested seasons "
                                        f"{seasons_to_monitor} for "
                                        f"{series_title}")
                                else:
                                    app.logger.info(
                                        f"Initial add: Monitoring first "
                                        f"episode of "
                                        f"{len(episodes_to_monitor)} seasons "
                                        f"for {series_title}")

                            except (ValueError, TypeError):
                                # Fallback to pilot episode
                                episodes_to_monitor = [
                                    season1_episodes[0]['id']] if season1_episodes else []
                                app.logger.warning(
                                    f"Invalid get_count, defaulting to pilot episode for {series_title}")
                        ...
                        if episodes_to_monitor:
                            ...
                            if monitor_response.ok:
                                ...
                                # Search for episodes if action_option is
                                # 'search'
                                if action_option == 'search':
                                    ...
                                    if search_response.ok:
                                        app.logger.info(
                                            f"✓ Started search for {
                                                len(episodes_to_monitor)} episodes of {series_title}")
                                    else:
                                        app.logger.error(
                                            f"Failed to search for episodes: {
                                                search_response.text}")
                            else:
                                app.logger.error(
                                    f"Failed to monitor episodes: {
                                        monitor_response.text}")
                        else:
                            app.logger.warning(
                                f"No episodes to monitor for {series_title}")
                else:
                    app.logger.error(
                        f"Failed to get episodes for series: {
                            episodes_response.text}")
            except Exception as e:
                app.logger.error(
                    f"Error executing default rule for {series_title}: {
                        str(e)}", exc_info=True)

        elif has_episeerr_select:
            app.logger.info(
                f"Processing {series_title} with episeerr_select tag - creating selection request")
            ...
            app.logger.info(
                f"✓ Created episode selection request for {series_title}")
        ...
        app.logger.error(
            f"Error processing Sonarr webhook: {
                str(e)}", exc_info=True)
        return jsonify({"status": "error", "message": str(e)}), 500
...
def process_seerr_webhook():
    ...
    try:
        ...
        if media_type != 'tv':
            app.logger.info(
                f"Request is not a TV show (media_type={media_type}), skipping")
            return jsonify(
                {"status": "success", "message": "Not a TV request"}), 200
        ...
        for item in extra_data:
            if item.get('name') == 'Requested Seasons':
                ...
                try:
                    ...
                    if ',' in seasons_value:
                        requested_seasons = [int(s.strip())
                                             for s in seasons_value.split(',')]
                    ...
                    app.logger.warning(
                        f"Could not parse requested seasons '{seasons_value}': {e}")
                break

        app.logger.info(
            f"TVDB ID: {tvdb_id}, TMDB ID: {tmdb_id}, Title: {title}, Requested Seasons: {requested_seasons}")
        ...
        if tvdb_id and request_id:
            ...
            app.logger.info(
                f"✓ Stored Jellyseerr request {request_id} for TVDB ID {tvdb_id_str} ({title})")
        else:
            app.logger.warning(
                f"Missing required data - TVDB ID: {tvdb_id}, Request ID: {request_id}")

        return jsonify({"status": "success"}), 200

    except Exception as e:
        app.logger.error(
            f"Error processing Jellyseerr webhook: {
                str(e)}", exc_info=True)
        return jsonify({"status": "error", "message": str(e)}), 500
...
def handle_server_webhook():
    ...
    if data:
        try:
            ...
            if result.returncode != 0:
                app.logger.error(
                    f"media_processor.py failed with return code {
                        result.returncode}")
                if result.stderr:
                ...
                if result.stderr:
                    app.logger.info(
                        f"media_processor.py output: {
                            result.stderr}")

            app.logger.info(
                "Webhook processing completed - activity tracked, next content processed")
            return jsonify({'status': 'success'}), 200
        except Exception as e:
        ...
        if notification_type == 'SessionStart':  # CHANGED: Use SessionStart instead of PlaybackStart
            ...
            if item_type == 'Episode':
                ...
                if all([series_name, season is not None, episode is not None]):
                    app.logger.info(
                        f"📺 Jellyfin session started: {series_name} S{season}E{episode} (User: {user_name})")
                    app.logger.info(
                        "This handles both NEW episodes and RESUMED episodes")
                    ...
                    try:
                        ...
                        polling_started = start_jellyfin_polling(
                            webhook_id, episode_info)

                        if polling_started:
                            app.logger.info(
                                f"✅ Started polling for {series_name} S{season}E{episode}")
                            return jsonify({
                                'status': 'success',
                            ...
                            app.logger.warning(
                                "Failed to start polling (may already be active)")
                            return jsonify(
                                {'status': 'warning', 'message': 'Polling may already be active'}), 200

                    except Exception as e:
                        app.logger.error(
                            f"Error starting Jellyfin polling: {
                                str(e)}")
                        return jsonify(
                            {'status': 'error', 'message': f'Failed to start polling: {str(e)}'}), 500
                else:
                    missing_fields = []
                    if not series_name:
                        missing_fields.append('SeriesName')
                    if season is None:
                        missing_fields.append('SeasonNumber')
                    if episode is None:
                        missing_fields.append('EpisodeNumber')

                    app.logger.warning(
                        f"Missing required fields: {missing_fields}")
                    return jsonify(
                        {'status': 'error', 'message': f'Missing fields: {missing_fields}'}), 400
            else:
                app.logger.info(
                    f"Item type '{item_type}' is not an episode, ignoring session start")
                return jsonify(
                    {'status': 'success', 'message': 'Not an episode'}), 200
            ...
            app.logger.info(
                f"📺 Jellyfin playback stopped: {series_name} S{season}E{episode} (User: {user_name})")
            ...
            if all([series_name, season is not None, episode is not None]):
                try:
                    ...
                    if stopped:
                        app.logger.info(
                            f"🛑 Stopped polling for {series_name} S{season}E{episode}")
                        return jsonify(
                            {'status': 'success', 'message': f'Stopped polling for {series_name}'}), 200
                    else:
                        app.logger.info(
                            f"ℹ️ No active polling found for {series_name} S{season}E{episode}")
                        return jsonify(
                            {'status': 'success', 'message': 'No active polling found'}), 200

                except Exception as e:
                    app.logger.error(
                        f"Error stopping Jellyfin polling: {
                            str(e)}")
                    return jsonify(
                        {'status': 'error', 'message': f'Failed to stop polling: {str(e)}'}), 500
            else:
                app.logger.warning(
                    "📺 Jellyfin playback stopped but missing episode info")
                return jsonify(
                    {'status': 'success', 'message': 'Playback stopped (missing episode info)'}), 200

        else:
            app.logger.info(
                f"Jellyfin notification type '{notification_type}' not handled")
            return jsonify(
                {'status': 'success', 'message': f'Notification type {notification_type} not handled'}), 200

    except Exception as e:
        app.logger.error(
            f"Failed to process Jellyfin webhook: {
                str(e)}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
def debug_series(series_id):
    ...
    try:
        ...
        if not rule:
            return jsonify(
                {"error": f"Series {series_id} not found in any rule"})
        ...
        if activity_date:
            days_since = (int(time.time()) - activity_date) / (24 * 60 * 60)
        else:
            days_since = "No activity date"
        return jsonify({
            "series_id": series_id,
            "rule_name": rule_name,
            "rule_config": rule,
...
def test_cleanup(series_id):
    ...
    try:
        ...
        if not rule:
            return jsonify(
                {"status": "error", "message": "Series not assigned to any rule"}), 404
...
def initialize_episeerr():
    ...
    try:
        ...
        if started:
            app.logger.info(
                "✅ Jellyfin active polling started (every 15 minutes)")
        else:
            app.logger.info(
                "⏭️ Jellyfin not configured - active polling disabled")
    except Exception as e:
