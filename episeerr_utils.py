...
AUTO_CREATE_TAGS = os.getenv(
    'EPISEERR_AUTO_CREATE_TAGS',
    'false').lower() == 'true'

# Log the setting at startup
if AUTO_CREATE_TAGS:
    logger.info(
        "Auto-create tags ENABLED - will create episeerr_default and episeerr_select tags")
else:
    logger.info(
        "Auto-create tags DISABLED - please create tags manually if using tag-based workflows")
...
def create_episeerr_default_tag():
    ...
    if EPISEERR_DEFAULT_TAG_ID is not None:
        logger.debug(
            f"'episeerr_default' tag ID already set: {EPISEERR_DEFAULT_TAG_ID}")
        return EPISEERR_DEFAULT_TAG_ID
    ...
    try:
        ...
        if tag_create_response.ok:
            ...
            logger.error(
                f"Failed to create episeerr_default tag. Status: {
                    tag_create_response.status_code}, Response: {
                    tag_create_response.text}")
            return None

    except requests.Timeout:
        logger.error(
            "Request to Sonarr timed out while creating 'episeerr_default' tag")
        return None
    except Exception as e:
...
def create_episeerr_select_tag():
    ...
    if EPISEERR_SELECT_TAG_ID is not None:
        logger.debug(
            f"'episeerr_select' tag ID already set: {EPISEERR_SELECT_TAG_ID}")
        return EPISEERR_SELECT_TAG_ID
    ...
    try:
        ...
        if tag_create_response.ok:
            ...
            logger.error(
                f"Failed to create episeerr_select tag. Status: {
                    tag_create_response.status_code}, Response: {
                    tag_create_response.text}")
            return None
        ...
        logger.error(
            "Request to Sonarr timed out while creating 'episeerr_select' tag")
        return None
    except Exception as e:
...
def initialize_episeerr():
    ...
    if AUTO_CREATE_TAGS:
        ...
        if default_tag_id is None:
            logger.warning(
                "Failed to initialize 'episeerr_default' tag. Tag-based workflows may not function.")
        else:
            logger.info(
                f"Initialized 'episeerr_default' tag with ID {default_tag_id}")

        logger.debug("Creating episeerr_select tag")
        select_tag_id = create_episeerr_select_tag()
        if select_tag_id is None:
            logger.warning(
                "Failed to initialize 'episeerr_select' tag. Episode selection workflows may not function.")
        else:
            logger.info(
                f"Initialized 'episeerr_select' tag with ID {select_tag_id}")
    else:
        ...
        if EPISEERR_DEFAULT_TAG_ID is None and EPISEERR_SELECT_TAG_ID is None:
            logger.warning(
                "No episeerr tags found. Please create 'episeerr_default' "
                "and 'episeerr_select' tags manually in Sonarr.")
            logger.info(
                "Or set EPISEERR_AUTO_CREATE_TAGS=true to auto-create them.")
...
def unmonitor_series(series_id, headers):
    ...
    try:
        ...
        if not episodes_response.ok:
            logger.error(
                f"Failed to get episodes. Status: {
        ...
        if all_episode_ids:
            ...
            if not unmonitor_response.ok:
                logger.error(
                    f"Failed to unmonitor episodes. Status: {
                        unmonitor_response.status_code}")
                return False
            else:
                logger.info(
                    f"Unmonitored all episodes in series ID {series_id}")
                return True
        else:
    ...
    try:
        ...
        if season_episode_ids:
            ...
            if not unmonitor_response.ok:
                ...
                logger.info(
                    f"Unmonitored all episodes in series ID {series_id} season {season_number}")
                return True
        else:
            logger.info(
                f"No episodes found for series ID {series_id} season {season_number}")
            return True
...
def cancel_download(queue_id, headers):
    ...
    try:
        ...
        if not response.ok:
            logger.warning(
                f"Bulk removal failed for queue item {queue_id}. Trying alternative method.")
            response = requests.delete(
...
def monitor_specific_episodes(
    ...
    try:
        ...
        target_episodes = [
            ...
            and ep['episodeNumber'] in episode_numbers
        ]

        if not target_episodes:
            logger.error(
                f"Target episodes {episode_numbers} not found in season {season_number}")
            return False
        ...
        if not monitor_response.ok:
            logger.error(
                f"Failed to monitor episodes. Status: {
                    monitor_response.status_code}")
            return False
        else:
            logger.info(
                f"Monitoring episodes {episode_numbers} in season {season_number}")
            return True

    except Exception as e:
        logger.error(
            f"Error monitoring specific episodes: {
                str(e)}", exc_info=True)
        return False
...
def search_episodes(series_id, episode_ids, headers):
    ...
    try:
        ...
        if search_response.ok:
            ...
            logger.error(
                f"Failed to trigger search. Status: {
...
def delete_overseerr_request(request_id):
    ...
    try:
        ...
        overseerr_url = os.getenv(
            'OVERSEERR_URL') or os.getenv('JELLYSEERR_URL')

        # Check if URL is configured
        if not overseerr_url:
            logger.warning(
                f"No Overseerr/Jellyseerr URL configured - skipping deletion of request {request_id}")
        ...
        logger.debug(
            f"Delete Request Response Status: {
                delete_response.status_code}")
        try:
            response_json = delete_response.json()
            logger.debug(
                f"Delete Request Response JSON: {
                    json.dumps(
                        response_json,
                        indent=2)}")
        except ValueError:
            logger.debug(
                f"Delete Request Response Text: {
                    delete_response.text}")

        if delete_response.ok or delete_response.status_code == 404:
            # 404 means the request was already deleted
            logger.info(
                f"Successfully deleted or request not found: {request_id}")
            return True
        else:
            logger.error(
                f"Failed to delete request {request_id}. Status: {
                    delete_response.status_code}")
            return False

    except Exception as e:
        logger.error(
            f"Error deleting Jellyseerr request: {
                str(e)}", exc_info=True)
        return False
...
def process_episode_selection(series_id, episode_numbers):
    ...
    try:
                    ...
                    try:
                        with open(os.path.join(REQUESTS_DIR, filename), 'r') as f:
                            ...
                            if request_data.get('series_id') == series_id:
                                ...
                                logger.info(
                                    f"Found season {season_number} from request file for series {series_id}")
                                break
                    except Exception as e:
        ...
        if season_number is None:
            ...
            if seasons_response.ok:
                ...
                if 'seasons' in series_data and series_data['seasons']:
                    ...
                    seasons = sorted([s.get('seasonNumber', 0)
                                     for s in series_data['seasons']])
                    if seasons:
                        season_number = seasons[-1]
                        logger.info(
                            f"Using latest season {season_number} for series {series_id}")
        ...
        logger.info(
            f"Processing episode selection for {
                series['title']} Season {season_number}: {episode_numbers}")
        ...
        for num in episode_numbers:
            if any(ep['episodeNumber'] == num for ep in episodes):
                ...
                logger.warning(
                    f"Episode {num} not found in {
                        series['title']} Season {season_number}")

        if not valid_episode_numbers:
            logger.error(
                f"No valid episodes found for selection {episode_numbers}")
            return False
        ...
        if not episode_ids:
            logger.error(
                f"Failed to find episode IDs for {valid_episode_numbers}")
            return False
        ...
        if search_success:
            logger.info(
                f"Successfully set up monitoring and search for {
                    len(valid_episode_numbers)} episodes")
            return True
        else:
            logger.error("Failed to search for episodes")
            return False
...
def process_episode_selection_with_season(
    ...
    try:
        ...
        logger.info(
            f"Processing episode selection for series {series_id}, season {season_number}, episodes {episode_numbers}")
        ...
        for num in episode_numbers:
            matching_episode = next(
                (ep for ep in episodes if ep['episodeNumber'] == num), None)
            if matching_episode:
                valid_episode_numbers.append(num)
                episode_ids_to_monitor.append(matching_episode['id'])
                logger.info(
                    f"Found episode {num}: ID {
                        matching_episode['id']} - {
                        matching_episode.get(
                            'title', 'Unknown')}")
            else:
                logger.warning(
                    f"Episode {num} not found in {
        ...
        logger.info(
            f"Monitoring episodes: {valid_episode_numbers} (IDs: {episode_ids_to_monitor})")
        ...
        logger.info(
            f"Successfully monitored {
                len(episode_ids_to_monitor)} episodes")
        ...
        if search_response.ok:
            logger.info(
                f"Successfully triggered search for {
                    len(episode_ids_to_monitor)} episodes")
            return True
        else:
            logger.error(
                f"Failed to search for episodes. Status: {
                    search_response.status_code}")
...
def check_and_cancel_unmonitored_downloads():
    ...
    try:
        ...
        queue_response = requests.get(
            f"{SONARR_URL}/api/v3/queue", headers=headers)

        if not queue_response.ok:
            logger.error(
                f"Failed to retrieve queue. Status: {
        ...
        for item in queue:
            ...
            logger.info(
                f"Examining queue item: {
                    item.get(
                        'title',
                        'Unknown')}")
            logger.info(
                f"Series ID: {
                    item.get('seriesId')}, Episode ID: {
                    item.get('episodeId')}")
            ...
            if item.get('seriesId') and item.get('episodeId'):
                ...
                has_episeerr_tag = (
                    EPISEERR_DEFAULT_TAG_ID in series.get('tags', [])
                    or EPISEERR_SELECT_TAG_ID in series.get('tags', [])
                )
                ...
                if has_episeerr_tag:
                    ...
                    if episode_info:
                        ...
                        logger.info(
                            f"Episode details: S{season_number}E{episode_number}, Monitored: {
                                episode_info.get('monitored')}")
                        ...
                        if series_id_str in pending_selections:
                            ...
                            selected_episodes = series_info.get(
                                'selected_episodes', set())
                            selection_season = series_info.get('season')

                            # If this episode is not in selected episodes for
                            # the selected season, cancel it
                            if season_number == selection_season and episode_number not in selected_episodes:
                                logger.info(
                                    f"Episode S{season_number}E{episode_number} not in "
                                    f"selected episodes {selected_episodes}. Cancelling download.")
                                should_cancel = True
                            elif season_number != selection_season:
                                logger.info(
                                    f"Episode is from season {season_number}, but selection is "
                                    f"for season {selection_season}. Cancelling download.")
                                should_cancel = True

                        # If not in pending_selections, check monitored status
                        elif not episode_info.get('monitored', False):
                            logger.info(
                                f"Episode S{season_number}E{episode_number} is unmonitored. Cancelling download.")
                            should_cancel = True

                        if should_cancel:
                            cancel_success = cancel_download(
                                item['id'], headers)
                            ...
                            if cancel_success:
                                ...
                                logger.error(
                                    f"Failed to cancel download for {
                                        series.get('title')} - Episode ID {
                                        item['episodeId']}")
                        else:
                            logger.info(
                                f"Episode S{season_number}E{episode_number} is monitored/selected - keeping download")
                    else:
                        logger.warning(
                            f"Could not get episode info for ID {
                                item['episodeId']}")
                else:
                    logger.info(
                        f"Series {
                            series.get('title')} does not have episeerr tags - skipping")

        # Log summary
        logger.info(
            f"Cancellation check complete. Cancelled {cancelled_count} unmonitored downloads for episeerr series")

    except Exception as e:
        logger.error(
            f"Error in download queue monitoring: {
                str(e)}", exc_info=True)
...
def process_series(tvdb_id, season_number, request_id=None):
    ...
    for attempt in range(12):  # Max 12 attempts
        try:
            ...
            if not response.ok:
                logger.error(
                    f"Failed to get series list. Status: {
            ...
            matching_series = [
                s for s in series_list if str(
                    s.get('tvdbId')) == str(tvdb_id)]

            if not matching_series:
                logger.warning(
                    f"No matching series found for TVDB ID {tvdb_id}")
                time.sleep(5)
                continue
            ...
            if EPISEERR_DEFAULT_TAG_ID not in series.get('tags', []):
                logger.info(
                    f"Series {
                        series['title']} does not have 'episeerr_default' tag. Skipping.")
                return False

            logger.info(f"Found series: {series['title']} (ID: {series_id})")

            # 1. Unmonitor episodes only for the requested season
            unmonitor_success = unmonitor_season(
                series_id, season_number, headers)

            if not unmonitor_success:
                logger.error(
                    f"Failed to unmonitor season {season_number} for series {series_id}")
                return False
    ...
    logger.error("Series not found after 12 attempts")
    return False
